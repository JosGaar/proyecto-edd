...
public class NatureReserveManager {
    ...
    private LinkedList<ParkRanger> parkRangers;
    ...
    public LinkedList<ParkRanger> getParkRangers()
    {
        return parkRangers;
    }
    ...
    public boolean addIncident(Incident newIncident)
    {
        ...
        if (!this.incidents.contains(criteriaByCodeIncident))
        {
            ...
            {
                newIncident.getArea().getIncidents().addByLast(newIncident);
                return true;
            } else
            {
                return false;
    ...
    private LinkedList<Visit> getVisitsActive()
    {
        Criteria<Visit> criteriaForVisit = visit -> visit.getVisibility() == Visibility.VISIBLE;
        LinkedList<Visit> visitsVisible = this.visits.getElements(criteriaForVisit);
        return visitsVisible;
    }

    public LinkedList<Visitor> getVisitorsActive()
    {
        LinkedList<Visitor> activeVisitors = new LinkedList<>();
        LinkedList<Visit> activeVisits = getVisitsActive();

        for (Visit visit : activeVisits)
        {
            if (visit.getVisitor().getStatus() != VisitStatus.INACTIVE)
            {
                activeVisitors.addByLast(visit.getVisitor());
            }
        }
    ...
    public boolean atenderIncident(Incident in, ParkRanger parkRangerIdSelect, LocalDateTime dateAttention)
    {
        ...
        Incident incidentToAttend = pendingIncidents.getElement(incident -> incident.getIncidentCode().equals(in.getIncidentCode()));
        ...
        ParkRanger parkRangerToAssign = freeParkRangers.getElement(parkRanger -> parkRanger.getIdentification().equals(parkRangerIdSelect.getIdentification()));
        ...
        if (!incidentToAttend.getDateTimeReport().isBefore(dateAttention))
        {
            return false;
        }
    ...
    {
        LinkedList<Incident> incidentsVisible = this.getVisbleIncidents();
        return incidentsVisible.getElements(incident -> incident.getIncidentStatus() == IncidentStatus.IN_PROCESS);
    }
